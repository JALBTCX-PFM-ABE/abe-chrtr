  ...
  while (bad)
    {
      ...
      switch (filetype)
          ...
          if (read_llz (llz_handle, LLZ_NEXT_RECORD, &llz_rec))
            {
              ...
              xyz->y = llz_rec.xy.lat * 60.0;
              xyz->x = llz_rec.xy.lon * 60.0;
              xyz->z = llz_rec.depth;
        ...
        case RDP_FILE:
          ...
          if ((rdp_record[0] == 0 && rdp_record[1] == 0 && rdp_record[2] == 0) || status <= 0)
            ...
            {
              xyz->y = rdp_record[0] / 10000000.0 * 60.0;
              xyz->x = rdp_record[1] / 10000000.0 * 60.0;
        ...
        case HOF_FILE:
          ...
          if ((hof.status & AU_STATUS_DELETED_BIT) || (hof.abdc < 70) || (hof.correct_depth == -998.0))
            ...
            {
              xyz->y = hof.latitude * 60.0;
              xyz->x = hof.longitude * 60.0;
        ...
        case TOF_FILE:
          ...
          if (tof.elevation_last == -998.0 || tof.conf_last < 50)
            ...
            {
              xyz->y = tof.latitude_last * 60.0;
              xyz->x = tof.longitude_last * 60.0;
        ...
        case PFM_FILE:
          ...
          if (!(depth_record[rec].validity & (PFM_INVAL | PFM_DELETED | PFM_REFERENCE)))
            {
              xyz->y = depth_record[rec].xyz.y * 60.0;
              xyz->x = depth_record[rec].xyz.x * 60.0;
      ...
      if (old_percent != percent)
        {
          fprintf (stderr, "%3d%% processed - %15f %15f %15f   \r", percent, xyz->y / 60.0, xyz->x / 60.0, xyz->z);
          fflush (stderr);
          old_percent = percent;
        }
    }
